...
extern "C"
{
...
#else
		if ( renderer_type == GEMGL_CORE_32 )
		{
	...
	int gemgl_startup( gemgl_interface_t * gl_interface, int renderer_type )
	{
		...
		gl_interface->type = renderer_type;
		...
		gemgl_parse_version( &major, &minor, gl_interface->type );
		

#if GEMGL_ENABLE_ES
#else
		if ( gl_interface->type == GEMGL_CORE_32 )
		{
